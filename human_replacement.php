...
class robot {
	...
	function run($command, $show_slots = true) {
        ...
        if($command_name == 'size') {
            ...
            if($size < $slot_count) {
                // we are losing slots. Time to bail and reset the whole thing
                /*
                $this -> slots='';
                $this -> slot_state = array();
                echo "truncating command_list because fear \n";
...
Start by creating some slots for the boxes by typing:
                
size [n]

where [n] is the number of slots you would like\n\n> ";
